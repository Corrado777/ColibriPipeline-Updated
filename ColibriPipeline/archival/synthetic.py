...
``generate_specific_lightcurve.py`` at the top level). The legacy import path
and the ``python generate_specific_lightcurve.py ...`` CLI both still work via
the ``generate_specific_lightcurve.py`` back-compat shim, which delegates to
``_cli()`` here.
"""
...
def findMatchedDir(obsdate, timestamp):
    ...
    # Set Green's basedir
    if cfg.current_telescope() == 'GREENBIRD':
