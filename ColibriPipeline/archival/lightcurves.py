...
Relocated into the ``archival`` package during the module-level refactor (was
``lightcurve_maker.py`` at the top level). The legacy import path still works
via the ``lightcurve_maker.py`` back-compat shim. The per-night writes here are
routed through ``archival.products.write_night_product`` -- a behaviour-
preserving indirection that reproduces the legacy bytes exactly.
"""
...
#            return -1
   ...
   #load in initial star positions from last image of previous minute
    ...
    write_night_product(posfile, initial_positions, kind='npy',
                        product='star_positions', telescope=telescope,
                        minute=minutefolder, detect_thresh=detect_thresh)
    ...
    for row in results:  # loop through each detected event
        ...
        #assemble file body, then route through the night-product write seam.
        #the body is built with the exact same writes/format as before, so the
        #bytes on disk are byte-for-byte identical to the legacy direct write.
        body_parts = []

        #file header
        body_parts.append('#\n#\n#\n#\n')
        body_parts.append('#    First Image File: %s\n' %(filenames[0]))
        body_parts.append('#    Star Coords: %f %f\n' %(star_coords[0], star_coords[1]))
        body_parts.append('#    DATE-OBS: %s\n' %(headerTimes[0][0]))
        body_parts.append('#    Telescope: %s\n' %(telescope))
        body_parts.append('#    Field: %s\n' %(field_name))
        body_parts.append('#    Error: %s\n' %(error))
        body_parts.append('#\n#\n#\n')
        body_parts.append('#filename     time      flux\n')
        ...
        for i in range(0, len(files_to_save)):
           # body_parts.append('%s %f  %f\n' % (files_to_save[i], float(headerTimes[i][0].split(':')[2].split('Z')[0]), star_save_flux[i]))
            body_parts.append('%s %f  %f\n' % (files_to_save[i], i, star_save_flux[i]))

        write_night_product(savefile, ''.join(body_parts), kind='text',
                            product='lightcurve_txt', telescope=telescope,
                            minute=minutefolder, star=row[2])
